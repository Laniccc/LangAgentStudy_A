...
flowchart TB
    ...
    START -->|phase=done 续问| FP[followup_prepare]
    FP --> A2[orchestrator_analyze]
    A2 -->|Send 并行| SF[sub_agent × N]
    SF --> BF[orchestrator_synthesize]
    BF --> FF[orchestrator_finalize_followup]
    FF --> END
...
| **多轮续问** | 追问+上轮终稿拼接 → 与首轮相同的主控分析+子 Agent ReAct → 汇总定稿（**跳过 reviewer**） |
...
| `search_loaded_image_vectors` | 在图片视觉 caption 向量索引中检索相关片段 |
| `read_loaded_image` | 精读某张图片的 caption（按需生成/按关键词摘录） |
| `list_loaded_images` | 列出已加载图片及预览 |
...
与 PDF 类似，支持在首问/续问时附带**架构图、曲线、表格截图**等：

| 方式 | 说明 |
|------|------|
| 放入目录 | 将图片复制到 `data/images/`，新会话默认自动加载 |
| 命令行 | `--image path/to/fig.png`（可多次） |
| `input.md` | 首行写 `IMAGE: a.png; b.jpg`（支持 `图片:` / `截图:`） |
| 续问追加 | 续问时在 `input.md` 增加 `IMAGE:` 行，保存后按回车 |
| Agent 检索 | `search_loaded_image_vectors(query)` — 在 caption 向量索引中定位相关片段 |
| Agent 深读 | `read_loaded_image(文件名, focus_hint)` — 精读某张图的解读文本 |

```bash
python main.py --mode research --image ./plots/eer_curve.png --task "..."
```

**说明**：

- **默认懒加载**：启动只登记路径；首次调用 `search_loaded_image_vectors` / `read_loaded_image` 时用视觉模型生成 caption，并写入与 PDF 同构的**本地稀疏向量索引**。
- 设 `IMAGE_CAPTION_ON_LOAD=1` 可恢复「启动时立即解读并建索引」。
- 流程建议：先 `search_loaded_image_vectors` 定位「哪张图、哪段描述」，再 `read_loaded_image` 精读。
- 需 `.env` 配置 `VISION_*`（见 `.env.example`）；未配置时写入占位说明，仍可登记文件名。
- 支持 png / jpg / jpeg / webp / gif / bmp，单张默认最大 8MB（`IMAGE_MAX_BYTES`）。
...
| PDF / 图片 | 正文中用 `PDF:`、`IMAGE:` 声明路径（与任务文字分开） |
